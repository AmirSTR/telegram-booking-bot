...
from db.database import (
    DB_PATH, get_master, get_client, register_client, get_services, get_service,
    get_booked_slots, create_booking, get_client_bookings,
    cancel_booking, add_to_waitlist, get_booking,
    update_client_name, update_client_phone, update_client_email, get_client_visit_count,
...
from utils.message_manager import MessageManager, safe_answer_callback
...
async def send_menu(callback: CallbackQuery, text: str, keyboard=None, parse_mode: str = "HTML"):
    await message_manager.edit_or_send(
        callback,
        text,
...
def _profile_prompt_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="◀️ Назад", callback_data="c:profile"),
            InlineKeyboardButton(text="🏠 Главное меню", callback_data="c:menu"),
        ]
    ])


async def _show_profile_screen(bot, chat_id: int, master_id: int, user_id: int, message_id: int | None = None):
    text, has_email = await _build_profile_text(bot, master_id, user_id)
    if message_id:
        await message_manager.edit_message(
            ...
            text,
            parse_mode="HTML",
            reply_markup=profile_kb(has_email),
            user_id=user_id,
        )
        return
...
async def _show_profile_input_screen(
    callback: CallbackQuery,
    state: FSMContext,
    prompt_text: str,
    next_state,
) -> None:
    await state.update_data(profile_message_id=callback.message.message_id if callback.message else None)
    await send_menu(callback, prompt_text, _profile_prompt_kb())
...
async def _handle_profile_update(
    ...
    *,
    ...
    if not validator(raw_value):
        if profile_message_id:
            await message_manager.edit_message(
                ...
                invalid_text,
                parse_mode="HTML",
                reply_markup=_profile_prompt_kb(),
                user_id=message.from_user.id,
            )
        else:
            await message_manager.answer(
                message,
                invalid_text,
                parse_mode="HTML",
                reply_markup=_profile_prompt_kb(),
    ...
    await updater(master_id, message.from_user.id, raw_value)
    await state.clear()
    await _show_profile_screen(
        message.bot,
        message.chat.id,
        master_id,
...
@router.callback_query(F.data == "__unused__")
async def cb_unknown(callback: CallbackQuery):
    await safe_answer_callback(
        callback,
        "Эта кнопка уже неактуальна. Откройте текущий экран заново.",
        show_alert=False,
    )
...
async def _send_profile_message(message: Message, master_id: int, user_id: int):
    await _show_profile_screen(message.bot, message.chat.id, master_id, user_id)


@router.callback_query(F.data == "c:profile")
async def cb_profile_v2(callback: CallbackQuery, state: FSMContext):
    master_id = await get_client_master_id(state)
    if not master_id:
        await safe_answer_callback(callback, "Сессия истекла.", show_alert=True)
        return
    await state.update_data(profile_message_id=callback.message.message_id if callback.message else None)
...
async def cb_edit_name(callback: CallbackQuery, state: FSMContext):
    ...
    if not master_id:
        await safe_answer_callback(callback, "Сессия истекла.", show_alert=True)
        return
    await _show_profile_input_screen(
        callback,
        state,
        "👤 Введите новое имя:",
...
async def cb_edit_phone_v2(callback: CallbackQuery, state: FSMContext):
    master_id = await get_client_master_id(state)
    if not master_id:
        await safe_answer_callback(callback, "Сессия истекла.", show_alert=True)
        return
    await _show_profile_input_screen(
        ...
        ProfileStates.editing_phone,
    )
    await safe_answer_callback(callback)


@router.callback_query(F.data == "c:edit_email")
async def cb_edit_email_v2(callback: CallbackQuery, state: FSMContext):
    master_id = await get_client_master_id(state)
    if not master_id:
        await safe_answer_callback(callback, "Сессия истекла.", show_alert=True)
        return
...
async def process_new_name(message: Message, state: FSMContext):
    name = message.text.strip() if message.text else ""
    await _handle_profile_update(
        message,
        state,
        raw_value=name,
        validator=lambda value: len(value) >= 2,
        updater=update_client_name,
        invalid_text="❌ Введите имя длиной не меньше 2 символов.",
    )


@router.message(ProfileStates.editing_phone)
async def process_new_phone_v2(message: Message, state: FSMContext):
    phone = message.text.strip() if message.text else ""
    await _handle_profile_update(
        message,
        state,
        raw_value=phone,
        validator=lambda value: len(value) >= 5,
        updater=update_client_phone,
        invalid_text="❌ Введите корректный номер телефона.",
    )


@router.message(ProfileStates.editing_email)
async def process_new_email_v2(message: Message, state: FSMContext):
    email = message.text.strip() if message.text else ""
    await _handle_profile_update(
        message,
        state,
        raw_value=email,
        validator=lambda value: "@" in value and "." in value.split("@")[-1],
        updater=update_client_email,
        invalid_text="❌ Введите корректный email, например: <code>user@example.com</code>",
    )


@router.callback_query(F.data == "c:profile_back")
@router.callback_query(F.data == "c:menu")
async def cb_client_menu(callback: CallbackQuery, state: FSMContext):
    await cb_client_back(callback, state)


@router.callback_query(F.data == "__legacy_profile__")
...
@router.callback_query(F.data == "__legacy_edit_phone__")
...
@router.message(ProfileStates.editing_phone, F.text == "__legacy__")
...
@router.callback_query(F.data == "__legacy_edit_email__")
...
@router.message(ProfileStates.editing_email, F.text == "__legacy__")
...
async def cb_about_master(callback: CallbackQuery, state: FSMContext):
    ...
    if lat and lon:
        try:
            await message_manager.send_location(
                callback.bot,
...
async def cb_unknown_fallback(callback: CallbackQuery):
    await safe_answer_callback(
        callback,
        "Эта кнопка уже неактуальна. Откройте текущий экран заново.",
        show_alert=False,
